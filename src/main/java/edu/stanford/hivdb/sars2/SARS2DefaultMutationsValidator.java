...
public class SARS2DefaultMutationsValidator implements MutationsValidator<SARS2> {
	...
	@Override
	public List<ValidationResult> validate(MutationSet<SARS2> mutations) {
	...
	private List<ValidationResult> validateNoStopCodons(MutationSet<SARS2> mutations) {
		List<ValidationResult> validationResults = new ArrayList<>();
	...
	private List<ValidationResult> validateNoTooManyUnusualMutations(MutationSet<SARS2> mutations) {
		List<ValidationResult> validationResults = new ArrayList<>();
		MutationSet<SARS2> unusualMutations = mutations.getUnusualMutations();
		int numUnusual = unusualMutations.size();

		MutationSet<SARS2> unusualMutAtDRP = unusualMutations.getAtDRPMutations();
		...
		if (numUnusual > 1) {
			if (numUnusualAtDRP == 0) {
				...
				MutationSet<SARS2> additionalMuts = unusualMutations.subtractsBy(unusualMutAtDRP);
				validationResults.add(newValidationResult(
	...
	private List<ValidationResult> validateNotApobec(MutationSet<SARS2> mutations) {
		MutationSet<SARS2> apobecs = mutations.getApobecMutations();
		MutationSet<SARS2> apobecDRMs = mutations.getApobecDRMs();
