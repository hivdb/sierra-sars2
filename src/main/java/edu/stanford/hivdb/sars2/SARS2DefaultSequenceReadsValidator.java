...
public class SARS2DefaultSequenceReadsValidator implements SequenceReadsValidator<SARS2> {
	...
	public List<ValidationResult> validate(SequenceReads<SARS2> seqReads) {
	...
	protected List<ValidationResult> validateNotEmpty(SequenceReads<SARS2> seqReads) {
		if (seqReads.isEmpty()) {
	...
	protected List<ValidationResult> validateTooLowThreshold(SequenceReads<SARS2> seqReads) {
		List<ValidationResult> results = new ArrayList<>();
	...
	protected List<ValidationResult> validateTrimmedPositions(SequenceReads<SARS2> seqReads) {
		List<ValidationResult> results = new ArrayList<>();
		List<OneCodonReadsCoverage<SARS2>> crcs = seqReads.getCodonReadsCoverage();
	...
	protected List<ValidationResult> validateNoMissingPositions(SequenceReads<SARS2> seqReads) {
		List<ValidationResult> results = new ArrayList<>();
		List<OneCodonReadsCoverage<SARS2>> crcs = seqReads.getCodonReadsCoverage();
		if (crcs.isEmpty()) {
			return results;
		}
		GenePosition<SARS2> leftMost = crcs.get(0).getGenePosition();
		GenePosition<SARS2> rightMost = crcs.get(crcs.size() - 1).getGenePosition();
		Set<GenePosition<SARS2>> needGenePositions = (
			GenePosition.getGenePositionsBetween(leftMost, rightMost)
		);
		Set<GenePosition<SARS2>> needDRGenePositions = GenePosition.getDRGenePositionsBetween(leftMost, rightMost);
		Set<GenePosition<SARS2>> availableGenePositions = (
	...
	protected List<ValidationResult> validateNoStopCodons(SequenceReads<SARS2> seqReads) {
		List<ValidationResult> results = new ArrayList<>();
		List<GeneSequenceReads<SARS2>> allGeneSeqReads = seqReads.getAllGeneSequenceReads();
		MutationSet<SARS2> stopCodons = new MutationSet<>();

		for (GeneSequenceReads<SARS2> gsr : allGeneSeqReads) {
	...
	protected List<ValidationResult> validateNoTooManyUnusualMutations(SequenceReads<SARS2> seqReads) {
		List<ValidationResult> results = new ArrayList<>();
		List<GeneSequenceReads<SARS2>> allGeneSeqReads = seqReads.getAllGeneSequenceReads();
		...
		for (GeneSequenceReads<SARS2> gsr : allGeneSeqReads) {
			numUnusuals += (
				...
				.stream()
				.mapToInt(pcr -> {
					for (CodonReads<SARS2> cr : pcr.getCodonReads(true, 1., cutoff)) {
						if (cr.isUnusual()) {
							return 1;
						}
					}
	...
	protected List<ValidationResult> validateNoTooManyApobec(SequenceReads<SARS2> seqReads) {
		List<ValidationResult> results = new ArrayList<>();
		List<GeneSequenceReads<SARS2>> allGeneSeqReads = seqReads.getAllGeneSequenceReads();
		int numAPOBECs = 0;
		MutationSet<SARS2> apobecDRMs = new MutationSet<>();
		double cutoff = seqReads.getMinPrevalence();
